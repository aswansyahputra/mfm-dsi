...
```{r, echo=FALSE}
knitr::include_graphics("./images/header2.png")
```
...
Fungsi **c** (c adalah kependekan dari Combine) membuat sebuah vektor baru dengan menggabungkan sekumpulan nilai. Sekarang coba buat vektor yang berisi karakter:

```{r}
c("a", "b", "v")
```

Vektor tidak dapat menyimpan nilai dengan tipe yang berbeda, coba buat vector dengan tipe yang berbeda-beda:
...
Semua nilai dikonversi ke satu tipe yaitu karakter, sehingga vektor akan dapat menampung semuanya.
...
Cara lain untuk membuat vector yang berisi urutan angka adalah dengan fungsi **seq**. Coba kita lakukan hal yang sama dengan diatas dengan **seq**,

```{r}
seq(5, 9)
```

**seq**, juga memungkinkan teman-teman untuk menggunakan *increaments* selain 1. Cobalah dengan 0,5:

```{r}
seq(5, 9, by = 0.5)
```

Sekarang coba buat vector dengan bilangan bulat dari 9 ke 5 (secara menurun)
...
Pada banyak bahasa pemrograman indeks array dimulai dengan 0, tapi di bahasa pemrograman R, indeks array dimulai dari 1. Untuk mendapatkan nilai pertama, dapat menggunakan cara: 
...
Teman-teman dapat menetapkan nilai baru dalam vektor yang ada. Cobalah ubah nilai indeks ketiga menjadi "dog": 
...
Kita juga bisa menambahkan nilai baru di akhir vector. Mari menambahkan kata keempat: 
...
Teman-teman dapat menggunakan kurung siku untuk mengakses banyak nilai dalam sebuah vector. Coba dapatkan nilai indeks ke-1 dan ke-3:
...
## 2.4 Vector Names
Untuk *challenge* kali ini, buatlah vektor yang berisi 3 nilai dengan tipe karakter/string dan simpan dalam variabel **ranks**.
Teman-teman dapat menetapkan nama ke elemen vektor dengan melewatkan vektor kedua dengan fungsi **names**, seperti:
...
Menetapkan nama untuk sebuah vektor dapat berguna sebagai label untuk data. Di bawah ini, teman-teman dapat melihat seperti apa vektor kita sekarang.
Teman-teman juga dapat menggunakan nama untuk mengakses nilai-nilai vektor. Cobalah akses nilai indeks ke-1 dengan menggunakan **"first"**:

```{r}
ranks[["first"]]
```

Sekarang isi nilai saat ini untuk indeks "third" dengan nilai yang berbeda dengan sebelumnya:
...
Hal yang sama berlaku untuk pembagian, perkalian atau setiap operasi dasar aritmatika. Sekarang coba bagi vector kita dengan 2: 
...
Jika teman-teman menambahkan dua vektor, R akan menambahkan nilai dari kedua vector tersebut. Kita akan buat vector kedua dan menyimpannya di variabel **b**.
...
Perhatikan bahwa R tidak menguji apakah keseluruhan vector sama, tapi melakukan pengecekan satu-satu terhadap nilai vector satu sama lain.

```{r}
a < b
```

Fungsi juga dapat bekerja dengan nilai skalar ataupun vector, coba dapatkan nilai sinus untuk tiap elemen vector a:
